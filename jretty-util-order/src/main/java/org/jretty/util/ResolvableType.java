...
import org.jretty.util.SerializableTypeWrapper.TypeProvider;
...
public class ResolvableType implements Serializable {
    ...
    public ResolvableType[] getInterfaces() {
        Class<?> resolved = resolve();
        if (resolved == null || ArrayUtils.isNullOrEmpty(resolved.getGenericInterfaces())) {
    ...
    private Type resolveBounds(Type[] bounds) {
        if (ArrayUtils.isNullOrEmpty(bounds) || Object.class == bounds[0]) {
            return null;
        }
        return bounds[0];
    }
    ...
    private int calculateHashCode() {
        int hashCode = nullSafeHashCode(this.type);
        if (this.typeProvider != null) {
            hashCode = 31 * hashCode + nullSafeHashCode(this.typeProvider.getType());
        }
        if (this.variableResolver != null) {
            hashCode = 31 * hashCode + nullSafeHashCode(this.variableResolver.getSource());
        }
        if (this.componentType != null) {
            hashCode = 31 * hashCode + nullSafeHashCode(this.componentType);
        }
        return hashCode;
    }
    ...
    public String toString() {
        ...
        if (hasGenerics()) {
            result.append('<');
            result.append(ArrayUtils.toString(getGenerics(), ", "));
            result.append('>');
        }
