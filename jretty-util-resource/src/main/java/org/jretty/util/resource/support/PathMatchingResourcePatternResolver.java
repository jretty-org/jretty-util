...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
    ...
    private static final Logger logger = LogFactory.getLogger(PathMatchingResourcePatternResolver.class);
        ...
        if (classLoader instanceof URLClassLoader) {
            try {
                for (URL url : ((URLClassLoader) classLoader).getURLs()) {
                    try {
                        UrlResource jarResource = (UrlUtils.URL_PROTOCOL_JAR.equals(url.getProtocol()) ?
                                new UrlResource(url) :
                                new UrlResource(UrlUtils.JAR_URL_PREFIX + url + UrlUtils.JAR_URL_SEPARATOR));
    ...
    protected void addClassPathManifestEntries(Set<Resource> result) {
        try {
            ...
            for (String path : StringSplitUtils.splitIgnoreEmpty(
                ...
                try {
                    ...
                    if (prefixIndex == 1) {
                        ...
                        filePath = capitalize(filePath);
                    }
                    UrlResource jarResource = new UrlResource(UrlUtils.JAR_URL_PREFIX +
                            UrlUtils.FILE_URL_PREFIX + filePath + UrlUtils.JAR_URL_SEPARATOR);
    ...
    private boolean hasDuplicate(String filePath, Set<Resource> result) {
        ...
        try {
            return result.contains(new UrlResource(UrlUtils.JAR_URL_PREFIX + UrlUtils.FILE_URL_PREFIX +
                    duplicatePath + UrlUtils.JAR_URL_SEPARATOR));
        }
        catch (MalformedURLException ex) {
    ...
    protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
        ...
        for (Resource rootDirResource : rootDirResources) {
            ...
            if (rootDirUrl.getProtocol().startsWith(UrlUtils.URL_PROTOCOL_VFS)) {
                result.addAll(VfsResourceMatchingDelegate.findMatchingResources(rootDirUrl, subPattern, getPathMatcher()));
            }
            else if (UrlUtils.isJarURL(rootDirUrl) || isJarResource(rootDirResource)) {
    ...
    protected Set<Resource> doFindPathMatchingJarResources(Resource rootDirResource, URL rootDirURL, String subPattern)
        ...
        if (con instanceof JarURLConnection) {
            ...
            try {
                int separatorIndex = urlFile.indexOf(UrlUtils.WAR_URL_SEPARATOR);
                if (separatorIndex == -1) {
                    separatorIndex = urlFile.indexOf(UrlUtils.JAR_URL_SEPARATOR);
                }
                if (separatorIndex != -1) {
    ...
    protected JarFile getJarFile(String jarFileUrl) throws IOException {
        if (jarFileUrl.startsWith(UrlUtils.FILE_URL_PREFIX)) {
            try {
                return new JarFile(UrlUtils.toURI(jarFileUrl).getSchemeSpecificPart());
            }
            catch (URISyntaxException ex) {
                // Fallback for URLs that are not valid URIs (should hardly ever happen).
                return new JarFile(jarFileUrl.substring(UrlUtils.FILE_URL_PREFIX.length()));
            }
        }
        else {
    ...
    protected File[] listDirectory(File dir) {
        ...
        Arrays.sort(files, new Comparator<File>() {
            ...
            public int compare(File o1, File o2) {
                return o1.getName()!=o2.getName()?1:0;
            }
        });
        return files;
    }
