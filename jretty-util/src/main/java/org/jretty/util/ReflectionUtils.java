...
public class ReflectionUtils {
	...
	public static void shallowCopyFieldState(final Object src, final Object dest) throws IllegalArgumentException {
	   ...
	   /**
     * 在属性相同的两个标准JavaBean之间进行属性拷贝<br>
     * 比如：父类转子类——将父类的属性值拷贝给子类。（当然，只需要属性名称和类型一致就行了，不需要继承关系）
     * @param parentSource 父类对象[orginal obj]
     * @param sonTarget 子类对象[target obj]
     */
    public static void attributeClone(Object parentSource, Object sonTarget) throws Exception {
        Map<String, Field> map = ReflectionUtils.getAllFields(sonTarget.getClass());
        Iterator<Entry<String, Field>> it = map.entrySet().iterator();
        String name;
        // Object value;
        while (it.hasNext()) {
            name = it.next().getKey();
            setAttributeValue(sonTarget, name, getAttributeValue(parentSource, name));
            // if( getAttributeValue(sonTarget, name)==null ){
            // value = getAttributeValue(parentSource, name); //父类的value
            // if(value!=null) {
            // setAttributeValue(sonTarget, name, value);
            // }
            // }
        }
    }
    ...
    public static void setAttributeValue(Object instance, String attrName, Object attrValue) throws Exception {
        ...
        try {
            ...
            try {
                ...
                throw new BasicRuntimeException("can't set this filed({})'s value to {}", attrName, instance.getClass()
                        .getName());
            }
        }
    }
