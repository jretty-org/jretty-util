...
public class ZipOutputStream extends DeflaterOutputStream implements
        ...
        if (current != null) {
            ...
            case STORED:
                ...
                if (e.size != written - locoff) {
                    throw new ZipException("invalid entry size (expected " + e.size + " but got " + (written - locoff)
                            + " bytes)");
                }
                if (e.crc != crc.getValue()) {
                    throw new ZipException("invalid entry crc-32 (expected 0x" + Long.toHexString(e.crc)
    ...
    private void writeLOC(XEntry xentry) throws IOException {
        ...
        // the 1 line below, modified by zollty
        byte[] nameBytes = charset == null ? getUTF8Bytes(e.name) : e.name.getBytes(charset);// getGBKBytes(e.name);//getUTF8Bytes(e.name);
    ...
    /*
     * Write central directory (CEN) header for specified entry. REMIND: add support for file attributes
     */
    private void writeCEN(XEntry xentry) throws IOException {
        ...
        writeInt(e.csize); // compressed size
        writeInt(e.size); // uncompressed size
        // the follow 1 line modified by zollty
        byte[] nameBytes = charset == null ? getUTF8Bytes(e.name) : e.name.getBytes(charset);// getUTF8Bytes(e.name);
        ...
        if (e.comment != null) {
            // the follow 1 line modified by zollty
            commentBytes = charset == null ? getUTF8Bytes(e.comment) : e.comment.getBytes(charset);// getUTF8Bytes(e.comment);
    ...
    private void writeEND(long off, long len) throws IOException {
        ...
        if (comment != null) { // zip file comment
            // the follow 1 line modified by zollty
            byte[] b = charset == null ? getUTF8Bytes(comment) : comment.getBytes(charset);// getUTF8Bytes(comment);
    ...
    /*
     * Returns an array of bytes representing the UTF8 encoding of the specified String. 等价于：
     * StringUtils.changeEncode(s, "UTF-8").getBytes("UTF-8")
     */
